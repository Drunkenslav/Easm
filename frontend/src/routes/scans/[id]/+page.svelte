<script lang="ts">
	...
	import { formatDate, getStatusClass, getSeverityClass } from '$lib/utils';
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
	<div class="mb-6 flex items-center justify-between">
		<div>
			<button on:click={() => goto('/scans')} class="text-sm text-gray-500 hover:text-gray-700 mb-2">
				← Back to Scans
			</button>
			<h1 class="text-3xl font-bold text-gray-900">Scan #{scanId}</h1>
		</div>
	</div>

	{#if loading}
		<div class="card">
			<p class="text-center text-gray-500 py-8">Loading scan details...</p>
		</div>
	{:else if !scan}
		<div class="card">
	...
	{:else}
		...
		<div class="card mb-6">
			<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
				<div>
					<h3 class="text-sm font-medium text-gray-500 mb-2">Target</h3>
					<p class="text-lg font-semibold text-gray-900">{scan.target}</p>
				</div>
				<div>
				...
				<div>
					<h3 class="text-sm font-medium text-gray-500 mb-2">Started</h3>
					<p class="text-gray-900">{formatDate(scan.started_at || scan.created_at)}</p>
				</div>
				<div>
				...
				<div>
					...
					<p class="text-2xl font-bold text-gray-900">{scan.vulnerabilities_found || 0}</p>
				</div>
			...
			{#if scan.vulnerabilities_by_severity && Object.keys(scan.vulnerabilities_by_severity).length > 0}
				<div class="mt-6 pt-6 border-t border-gray-200">
					<h3 class="text-sm font-medium text-gray-500 mb-3">Severity Breakdown</h3>
		...
		<!-- Vulnerabilities -->
		<div class="card">
			<h2 class="text-lg font-semibold text-gray-900 mb-4">Discovered Vulnerabilities</h2>

			{#if vulnerabilities.length === 0}
				<p class="text-center text-gray-500 py-8">No vulnerabilities found</p>
			{:else}
				<div class="space-y-3">
					{#each vulnerabilities as vuln}
						<div class="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
							<div class="flex items-start justify-between mb-2">
								<div class="flex-1">
									<h3 class="font-medium text-gray-900">{vuln.name}</h3>
							...
							{#if vuln.description}
								<p class="text-sm text-gray-700 mt-2">{vuln.description}</p>
							{/if}

							<div class="flex items-center gap-2 mt-3">
								<span class="text-xs text-gray-500">Template: {vuln.template_id}</span>
		...
		{#if scan.nuclei_output}
			<div class="card mt-6">
				<h2 class="text-lg font-semibold text-gray-900 mb-4">Nuclei Output</h2>
				<pre class="bg-gray-50 p-4 rounded-lg overflow-x-auto text-xs text-gray-800">{scan.nuclei_output}</pre>
			</div>
		{/if}
	{/if}
</div>
